...
// content:"/Type /Font", nocase; distance:0; within:500;
...
static CursorInfo rule16343cursor7 =
{
   0,
...
{
    "url", /* type */
    "opensource.adobe.com/dc-acrobat-sdk-docs/pdfstandards/PDF32000_2008.pdf" /* value */
};
...
#ifndef MISSINGFILEDATA
    // Move to file_data.
    if(fileData(sp, rule16343options[7]->option_u.cursor, &cursor_normal) <= 0) {
        return RULE_NOMATCH;
    }
#endif
    ...
    // There is an extra indent here, but it's left to make a prettier diff.
        ...
        if (   processFlowbits(sp, rule16343options[1]->option_u.flowBit) > 0 ||
            ...
            while (contentMatch(sp, rule16343options[2]->option_u.content, &cursor_normal) > 0) {
                ...
                if (contentMatch(sp, rule16343options[6]->option_u.content, &cursor_normal) > 0) {
                    continue;
                }
